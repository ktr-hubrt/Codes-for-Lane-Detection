...
def test_lanenet_batch(image_dir, weights_path, batch_size, use_gpu, save_dir=None):
    ...
    with sess.as_default():
        ...
        for epoch in range(epoch_nums):
            ...
            # import pdb;pdb.set_trace()
            seg_gt_path = ['data/result_test/' +tmp.strip().replace('.jpg','_sur_new.png') for tmp in image_path_epoch]
            ...
            for index, binary_seg_image in enumerate(binary_seg_images):
                ...
                # import pdb;pdb.set_trace()
                single_class_iou = cluster.get_lane_mask_iou(
                                                   binary_seg_ret=mask_png_2,
                                                   binary_score_images=mask_png_2,
                ...
                # mask_image = cv2.resize(image_vis_list[index], (800,
                #                                      288),
                ...
                if save_dir is not None:
                    image_vis_list[index] = cv2.resize(image_vis_list[index], (800,288),
                                     interpolation=cv2.INTER_LINEAR)
                    mask_image = image_vis_list[index]
                    ...
                    f_list.write(' ')
                    f_list.write(str(np.min(single_class_iou[1:])))
                    f_list.write('\n')
                    
                    # import pdb;pdb.set_trace()
                    if np.min(single_class_iou[1:])<0.7:
                        cv2.imwrite(image_save_path.replace('.jpg','_raw.jpg'),mask_image)
                        cv2.imwrite(image_save_path.replace('.jpg','_pred.png'), mask_png_2*50)
                        cv2.imwrite(image_save_path.replace('.jpg','_gt.png'), seg_gt_list[index]*50)
                    # import pdb;pdb.set_trace()
                    ...
                    class_1_iou.update(np.mean(single_class_iou[1:]))
